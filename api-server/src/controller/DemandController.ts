...
router.get("/:id_demand", verifyToken, async (req: Request, res: Response) => {
    const {code, message, data} = await DemandService.getDemandById(req.params.id_demand);
    res.status(code).json({message, data})
})
...
router.delete("/:id_demand", verifyToken, async (req: Request, res: Response) => {
    const {code, message, data} = await DemandService.deleteDemand(req.params.id_demand);
    res.status(code).json({message, data})
})
